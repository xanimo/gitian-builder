...
1. Are named `Gitian-<suite>-<arch>` -- e.g. Gitian-bionic-i386 for a 32-bit, Ubuntu 16 machine.
...
```
    VBoxManage modifyvm Gitian-bionic-i386 --natpf1 "guestssh,tcp,,2223,,22"
```
...
If you have everything set-up properly, you should be able to:
    ...
    make-clean-vm --suite bionic --arch i386
    ...
    # For LXC:
    LXC_ARCH=i386 LXC_SUITE=bionic on-target ls -la

    # For KVM:
    start-target 32 bionic-i386 &
...
* To start the target VM run `start-target 32 bionic-i386` or `start-target 64 bionic-amd64`
